...
_ICON_BLUE = (0, 136, 204, 255)
_ICON_GREEN = (34, 197, 94, 255)
_ICON_GRAY = (120, 120, 120, 255)


def _make_status_icon(color: tuple, size: int = 64):
    """Render a circle tray icon in the given RGBA colour."""
    try:
        from utils.tray_common import make_icon_image
        return make_icon_image(size, color=color)
    except Exception:
        return load_icon()
...
def _on_show_stats(icon=None, item=None) -> None:
    try:
        ...
        msg = (
            ...
            f"Всего:        {_stats.connections_total}\n"
            f"WS:         {_stats.connections_ws}\n"
            f"TCP:        {_stats.connections_tcp_fallback}\n"
            f"CF:         {_stats.connections_cfproxy}\n"
            f"Ошибок:      {_stats.ws_errors}\n"
        ...
        msg = "Статистика недоступна"
    _show_info(msg, "Статистика прокси")
...
def _start_tray_updater() -> None:
    """Background thread that updates tray icon colour + tooltip every 5 s."""
    from proxy.stats import stats as _proxy_stats

    def _updater():
        while not _exiting:
            time.sleep(5)
            if _exiting or _tray_icon is None:
                break
            try:
                active = _proxy_stats.connections_active
                if active > 0:
                    color = _ICON_GREEN
                else:
                    color = _ICON_BLUE
                new_icon = _make_status_icon(color)
                _tray_icon.icon = new_icon
                _tray_icon.title = _proxy_stats.tray_tooltip()
            except Exception as exc:
                log.debug("Tray updater error: %s", exc)
