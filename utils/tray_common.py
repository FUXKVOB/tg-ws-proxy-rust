...
def apply_proxy_config(cfg: dict) -> bool:
    ...
    pc.fake_tls_domain = cfg.get("fake_tls_domain", DEFAULT_CONFIG["fake_tls_domain"])
    pc.proxy_protocol = cfg.get("proxy_protocol", DEFAULT_CONFIG["proxy_protocol"])
    pc.shutdown_timeout = max(0.5, cfg.get("shutdown_timeout", DEFAULT_CONFIG["shutdown_timeout"]))
    return True
...
def tg_proxy_url(cfg: dict) -> str:
    ...
    fake_tls = cfg.get("fake_tls_domain", "").strip()
    link_host = get_link_host(host)
    if fake_tls:
        domain_hex = fake_tls.encode('ascii').hex()
        return (
            f"tg://proxy?server={link_host}&port={port}"
            f"&secret=ee{secret}{domain_hex}"
        )
