...
_TIP_FAKE_TLS = (
    "Домен для маскировки трафика под обычный HTTPS (Fake TLS / ee-secret).\n"
    "Пример: example.com\n"
    "Если указан, прокси-ссылка будет начинаться с ee вместо dd.\n"
    "Оставьте пустым, чтобы не использовать Fake TLS."
)
_FAKE_TLS_HELP_URL = "https://github.com/Flowseal/tg-ws-proxy/blob/main/docs/FakeTLS.md"
...
class TrayConfigFormWidgets:
    ...
    fake_tls_domain_var: Optional[Any] = None
...
def install_tray_config_form(
    ...
    *,
    ...
    # ── Fake TLS ──────────────────────────────────────────────────────────────
    ftls_inner = _config_section(ctk, frame, theme, "Fake TLS (маскировка)")

    ftls_row = ctk.CTkFrame(ftls_inner, fg_color="transparent")
    ftls_row.pack(fill="x", pady=(0, 4))
    ftls_lbl = _label(ctk, ftls_row, theme, "SNI-домен (оставьте пустым для отключения)", size=11)
    ftls_lbl.pack(anchor="w", pady=(0, 2))

    ftls_input_row = ctk.CTkFrame(ftls_inner, fg_color="transparent")
    ftls_input_row.pack(fill="x")

    fake_tls_domain_var = ctk.StringVar(
        value=cfg.get("fake_tls_domain", default_config.get("fake_tls_domain", ""))
    )
    ftls_entry = _entry(
        ctk, ftls_input_row, theme, var=fake_tls_domain_var,
        height=32, radius=8,
    )
    ftls_entry.pack(side="left", fill="x", expand=True, padx=(0, 6))
    attach_tooltip_to_widgets([ftls_lbl, ftls_entry], _TIP_FAKE_TLS)

    ctk.CTkButton(
        ftls_input_row, text="?", width=28, height=32,
        font=(theme.ui_font_family, 14), corner_radius=8,
        fg_color=theme.tg_blue, hover_color=theme.tg_blue_hover,
        text_color="#ffffff", border_width=1, border_color=theme.field_border,
        command=lambda: webbrowser.open(_FAKE_TLS_HELP_URL),
    ).pack(side="right")

    hint_ftls = _label(
        ctk, ftls_inner, theme,
        "При изменении домена потребуется перезапуск прокси. "
        "Ссылка для Telegram начнётся с ee… вместо dd…",
        size=11, justify="left", wraplength=_INNER_W,
    )
    hint_ftls.pack(anchor="w", pady=(4, 0))
    ...
    # ── Live stats ─────────────────────────────────────────────────────────────
    stats_inner = _config_section(ctk, frame, theme, "Статистика")

    def _make_stats_text() -> str:
        try:
            from proxy.stats import stats as _stats
            from proxy.utils import human_bytes as _hb
            lines = [
                f"Время работы:      {_stats.uptime_str()}",
                f"Активных:          {_stats.connections_active}",
                f"Всего соединений:  {_stats.connections_total}",
                f"WS / TCP / CF:     {_stats.connections_ws} / "
                f"{_stats.connections_tcp_fallback} / {_stats.connections_cfproxy}",
                f"Плохих:            {_stats.connections_bad}",
                f"Ошибок WS:         {_stats.ws_errors}",
                f"Трафик ↑:          {_hb(_stats.bytes_up)}",
                f"Трафик ↓:          {_hb(_stats.bytes_down)}",
            ]
            if _stats.dc_connections:
                dc_parts = "  ".join(
                    f"DC{k}: {v}" for k, v in sorted(_stats.dc_connections.items())
                )
                lines.append(f"По DC:             {dc_parts}")
            pool_total = _stats.pool_hits + _stats.pool_misses
            if pool_total:
                lines.append(
                    f"Пул WS (попаданий):{_stats.pool_hits}/{pool_total}"
                )
            return "\n".join(lines)
        except Exception:
            return "Статистика недоступна"

    _stats_tb = ctk.CTkTextbox(
        stats_inner, width=_INNER_W, height=160,
        font=(theme.mono_font_family, 11), corner_radius=8,
        fg_color=theme.bg, border_color=theme.field_border,
        border_width=1, text_color=theme.text_secondary,
        activate_scrollbars=False,
        state="disabled",
    )
    _stats_tb.pack(fill="x", pady=(0, 6))

    _stats_refresh_id: list = [None]

    def _refresh_stats():
        try:
            _stats_tb.configure(state="normal")
            _stats_tb.delete("1.0", "end")
            _stats_tb.insert("1.0", _make_stats_text())
            _stats_tb.configure(state="disabled")
        except Exception:
            pass
        try:
            widget = _stats_tb
            _stats_refresh_id[0] = widget.after(2000, _refresh_stats)
        except Exception:
            pass

    _refresh_stats()

    _stats_btn_row = ctk.CTkFrame(stats_inner, fg_color="transparent")
    _stats_btn_row.pack(fill="x")
    ctk.CTkButton(
        _stats_btn_row, text="Сбросить счётчики", height=28,
        font=(theme.ui_font_family, 12), corner_radius=8,
        fg_color=theme.field_bg, hover_color=theme.field_border,
        text_color=theme.text_primary, border_width=1, border_color=theme.field_border,
        command=lambda: (__import__('proxy.stats', fromlist=['stats']).stats.reset_counters(), _refresh_stats()),
    ).pack(side="left")
    ...
    return TrayConfigFormWidgets(
        ...
        fake_tls_domain_var=fake_tls_domain_var,
...
def validate_config_form(
    ...
    *,
    ...
    if widgets.fake_tls_domain_var is not None:
        new_cfg["fake_tls_domain"] = widgets.fake_tls_domain_var.get().strip()
