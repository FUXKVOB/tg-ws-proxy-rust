...
async def _run(stop_event: Optional[asyncio.Event] = None):
    ...
    if proxy_config.fallback_cfproxy:
        ...
        if user:
            ...
            await start_cfproxy_domain_refresh()
        ...
        async with server:
            if stop_event:
                ...
                if stop_task in done:
                    ...
                    if _client_tasks:
                        log.info("Draining %d active connection(s)...", len(_client_tasks))
                        _drain_timeout = proxy_config.shutdown_timeout
                        done2, pending = await asyncio.wait(
                            _client_tasks, timeout=_drain_timeout,
                        )
                        if pending:
                            log.warning("Force-cancelling %d connection(s) after %.1fs",
...
def main():
    ...
    ap.add_argument('--shutdown-timeout', type=float, default=10.0,
                    metavar='SEC',
                    help='Seconds to wait for active connections during '
    ...
    proxy_config.shutdown_timeout = max(0.5, args.shutdown_timeout)
