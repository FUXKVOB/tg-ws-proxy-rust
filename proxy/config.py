...
async def start_cfproxy_domain_refresh() -> None:
    global _refresh_stop, _refresh_task

    if _refresh_task is not None:
        _refresh_stop.set()
        try:
            await asyncio.wait_for(_refresh_task, timeout=5)
    ...
    async def _loop():
        await loop.run_in_executor(None, refresh_cfproxy_domains)
