...
try:
    ...
    _MsgSplitterRust = tg_ws_proxy_rs.MsgSplitter
except ImportError:
    _MsgSplitterRust = None
...
# Drain the write buffer only when it exceeds this threshold (bytes).
# Keeps small packets batched in the kernel's TCP send buffer while
# still providing backpressure for large sustained downloads.
_WRITE_HIGH_WATER = 64 * 1024  # 64 KB
...
if _MsgSplitterRust is not None:
    MsgSplitter = _MsgSplitterRust
else:
    class MsgSplitter:
        """
        ...
        try:
            while True:
                ...
                if splitter:
                    ...
                    await ws.send(chunk)
                # Drain the WS writer to maintain backpressure toward Telegram
                if ws.writer.transport.get_write_buffer_size() > _WRITE_HIGH_WATER:
                    await ws.writer.drain()
    ...
    async def ws_to_tcp():
        ...
        try:
            while True:
                ...
                writer.write(data)
                # Smart drain: only flush when the write buffer is large enough
                # to make it worth the coroutine suspension overhead.
        ...
        try:
            while True:
                ...
                if is_up:
                    ...
                    data = ctx.clt_enc.update(plain)
                dst_w.write(data)
                # Smart drain: avoid per-packet coroutine suspensions
                if dst_w.transport.get_write_buffer_size() > _WRITE_HIGH_WATER:
                    await dst_w.drain()
